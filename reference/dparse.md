...
Note R-based dparser tree walking works on Windows (with R tools) Mac,
or Linux. Linking to arbitrary c grammars works on any platform.
...
- ...:
  ...
  with the exception of `use_r_header` which is forced to be `TRUE`.

## Value

A function that allows parsing of a file based on the grammar supplied.
This function would be able to parse arbitrary grammars the way you may
want with your own user supplied function.
