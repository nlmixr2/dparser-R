...
static OffsetEntry *search_for_offset(File *fp, char *name) {
  ...
  if (n) {
    ...
    for (i = h, j = 0; i < v->n && j < SET_MAX_SEQUENTIAL; i = ((i + 1) % n), j++) {
      if (!v->v[i]) {
        if (!(0)){Rf_error("Error parsing: assert(0).");};
        return 0;
      } else {
...
static OffsetEntry *get_offset(File *fp, char *name, ...) {
  ...
  (void)n;
  va_end(ap);
  if (!(n < 256 && n >= 0)){Rf_error("Error parsing: assert(n < 256 && n >= 0).");};
...
static void write_scanner_data(File *fp, Grammar *g, char *tag) {
  ...
  for (i = 0; i < g->terminals.n; i++) {
    ...
    if (t->regex_production && t->regex_production->rules.v[0]->speculative_code.code) {
      if (!(!fp->binary)){Rf_error("Error parsing: assert(!fp->binary).");};
    ...
    for (j = 0; j < ss->n; j++) {
      if (!s->same_shifts) {
        ...
        for (k = 0; k < g->scanner_blocks; k++) {
          ...
          xv = &vsblock[ivsblock];
          ivsblock++;
          if (!(ivsblock <= nvsblocks)){Rf_error("Error parsing: assert(ivsblock <= nvsblocks).");};
  ...
  for (i = 0; i < g->states.n; i++) {
    ...
    if (ss->n && !s->same_shifts) {
      ...
      if (!(sizeof(SB_uint8) == sizeof(SB_uint16) && sizeof(SB_uint16) == sizeof(SB_uint32))){Rf_error("Error parsing: assert(sizeof(SB_uint8) == sizeof(SB_uint16) && sizeof(SB_uint16) == sizeof(SB_uint32)).");};
      ...
      for (j = 0; j < ss->n; j++) {
        ...
        for (k = 0; k < g->scanner_blocks; k++) {
          ...
          xv = &vs;
          yv = set_add_fn(pscanner_block_hash, xv, &scanner_block_fns);
          if (!(yv != xv)){Rf_error("Error parsing: assert(yv != xv).");};
...
void write_parser_tables(Grammar *g, char *tag, File *file) {
  ...
  if (g->default_white_space) {
    if (!(!file->binary)){Rf_error("Error parsing: assert(!file->binary).");};
