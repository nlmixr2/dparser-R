...
save_binary_tables(File *file) {
  ...
  if (file->str) {
    file->cur_str = *file->str = (unsigned char*)R_chk_calloc(1,len);
    *file->str_len = len;
  }
...
init_buf(Buf *buf, int initial_size) {
  ...
  buf->start = R_chk_calloc(1,buf->len);
...
make_room_in_buf(Buf *buf, size_t size) {
  while (buf->cur + size > buf->start + buf->len) {
    ...
    buf->len = buf->len*2 + 1;
    buf->start = R_chk_realloc(buf->start, buf->len);
...
new_offset(File *fp, char *name) {
  OffsetEntry *entry = R_chk_calloc(1,sizeof(OffsetEntry));
...
write_scanner_data(File *fp, Grammar *g, char *tag) {
  ...
  vsblock = R_chk_calloc(1,(nvsblocks ? nvsblocks : 1) * sizeof(ScannerBlock));
  for (i = 0; i < 4; i++) {
...
write_goto_data(File *fp, Grammar *g, char *tag) {
  ...
  goto_valid = R_chk_calloc(1,nvalid_bytes);
  vec_clear(&vgoto);
  for (i = 0; i < g->states.n; i++) {
    s = g->states.v[i];
    if (s->gotos.n) {
...
fprintf(hfp, "	/*\n  Generated by R\'s mkdparse a port of Make DParser Version %s\n", ver);
	fprintf(hfp,"  R available at https://github.com/mattfidler/dparser-R\n");
                             ...
                             FILE *fp, unsigned char **str, unsigned int *str_len)
{
  ...
  if (!binary) {
    ...
    d_version(ver);
fprintf(fp, "	/*\n  Generated by R\'s mkdparse a port of Make DParser Version %s\n", ver);
	fprintf(fp,"  R available at https://github.com/mattfidler/dparser-R\n");
